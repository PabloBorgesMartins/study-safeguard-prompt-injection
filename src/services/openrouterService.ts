...
export class OpenRouterService {
  ...
  async checkGuardrails(userInput: string, enabled: boolean) {
    ...
    const isUnsafe = result.toUpperCase().startsWith('UNSAFE') || result.toUpperCase().startsWith('BLOCKED');

    if (isUnsafe) {
      return { safe: false, reason: 'Prompt injection detected by safe guard model.', analysis: result };
    }
