...
export const createGuardrailsCheckNode = (openRouterService: OpenRouterService) => {
  return async (state: GraphState): Promise<Partial<GraphState>> => {
    try {
      ...
      const systemPrompt = await template.format({ USER_ROLE: state.user.role, USER_NAME: state.user.username });

      const message = systemPrompt.concat('\n', userPrompt);

      const result = await openRouterService.checkGuardrails(message, state.guardrailsEnabled);
      ...
      return {
        guardrailCheck: {
          reason: 'Guardrails service unavailable or failed - request blocked for safety',
          safe: false,
        }
      };
    }
